...
import {FlashLoanReceiver} from "./FlashLoanReceiver.sol"; // ! Unused import
...
contract NaiveReceiverPool is Multicall, IERC3156FlashLender {
    ...
    constructor(address _trustedForwarder, address payable _weth, address _feeReceiver) payable {
        ...
        _deposit(msg.value); //Contract is deployed with initial funds
    }

    function maxFlashLoan(address token) external view returns (uint256) {
        if (token == address(weth)) return weth.balanceOf(address(this));
        return 0; // n Checking the max liquidity in the pool
    }
    ...
    {
        if (token != address(weth)) revert UnsupportedCurrency(); 
        // q What happens if the pool doesn't have enough weth to carry out the loan?
        ...
        totalDeposits += amountWithFee; // update the balance because the amount will be in the contract
